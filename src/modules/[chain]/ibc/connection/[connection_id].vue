...
function fetchSendingTxs(channel: string, port: string, pageNum = 0) {
  ...
  chainStore.rpc.getTxs("?order_by=1&events=send_packet.packet_src_channel='{channel}'&events=send_packet.packet_src_port='{port}'", { channel, port }, page.value).then(res => {
    txs.value = res
  })
...
function fetchRecevingTxs(channel: string, port: string, pageNum = 0) {
  ...
  port_id.value = port
  txs.value = {} as PaginatedTxs
  chainStore.rpc.getTxs("?order_by=1&events=recv_packet.packet_dst_channel='{channel}'&events=recv_packet.packet_dst_port='{port}'", { channel, port }, page.value).then(res => {
    txs.value = res
  })
    .finally(() => loading.value = false)
}
